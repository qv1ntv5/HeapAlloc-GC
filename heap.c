...
static void mark_region(const uintptr_t *start, const uintptr_t *end)
{
    for (; start < end; start += 1) {
        ...
        for (size_t i = 0; i < alloced_chunks.count; ++i) {
            ...
            printf("GC: --> Checking if %p exists in heap's segment [%p,%p).\n", p, chunk.start, chunk.start + chunk.size);
            if (chunk.start <= p && p < chunk.start + chunk.size) {
                printf("GC:     %p exists in the memory region, marking chunk as reachable.\n\n",p);
                ...
                printf("GC:     %p DO NOT exist in the memory region.\n\n",p);
...
void heap_collect()
{
    ...
    printf("\nGC: Checking stack's memory region: (%p,%p)\n\n",stack_start,stack_base + 1);
    ...
    for (size_t i = 0; i < to_free_count; ++i) {
        printf("\nGC: Freeing chunk [%p,%p). This chunk is now reusable in the heap.\n",to_free[i],to_free[i] + alloced_chunks.chunks[i].size);
        free(to_free[i]);
    }
}